...
\renewcommand{\huge}{\fontsize{8.83pt}{\baselineskip}\selectfont}
...
\author[\empty]{Jun Xu}
\author[\empty]{Lei Zhang}
\author[\empty]{David Zhang}
\affil[\empty]{Department of Computing, The Hong Kong Polytechnic University, Hong Kong, China}
...
\vspace{-2mm}
\section{\parbox[t]{10cm}{External Prior Guided Internal Prior Learn-\\ing and Image Denoising}}
...
\end{equation}
where $\bm{\lambda} = \frac{1}{2}[\lambda_{1},\lambda_{2},...,\lambda_{3p^2}]^{\top}$ is the vector of regularization parameter, $\text{sgn}(\bullet)$ is the sign function and $\odot$ means element-wise multiplication.\ The detailed derivation of Eq. (\ref{equ7}) can be found in the supplementary file.
\vspace{2mm}\\
...
\subsection{\parbox[t]{8cm}{Comparison among external, internal and 
\\guided internal priors}}
...
\subsection{\parbox[t]{10cm}{Comparison with State-of-the-Art Denoising 
\\Methods}}
