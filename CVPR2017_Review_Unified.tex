...
\mathbf{\Sigma}=\mathbf{U}_{e}\mathbf{S}_{e}\mathbf{U}_{e}^{T}.
\end{equation}
...
\end{equation}
Since dictionary $\mathbf{D}_{t}=[\mathbf{D}_{e}\ \mathbf{D}_{i}^{(t)}]$ is orthogonal, the problem (\ref{equ8}) has closed-form solution $\mathbf{A}^{(t)}=T_{\lambda}(\mathbf{D}_{t}^{T}\mathbf{Y})$, where $T_{\lambda}$ is the soft-thresholding function \cite{softthresholding} (we will give explicit solution in next section).
...
\subsection{Prior Weights for Weighted Sparse Coding}

Under the sparse coding framework \cite{ksvd} in Equ. (\ref{equ6}), the dictionary atoms are treated equally for each subspace. This ignores the external weight prior learned from clean images. The singular values in the diagonal of $\mathbf{S}_{e}$ (Equ. (\ref{equ3})) reflect the significance of the singular vectors in $\mathbf{U}_{e}$. Therefore, similar to \cite{pgpd}, we employ a weighted sparse coding framework for real noisy image denoising problem. Given a real noisy image, we extract the internal PGs  from it and assign the noisy PGs into different subspaces via Equ. (\ref{equ5}). For each subspace, the learned orthogonal dictionary is denoted as $\mathbf{D}=[\mathbf{D}_{e}\ \mathbf{D}_{i}]$, and the PGs in this subspace is denoted as $\mathbf{Y}$. We employ the square root of singular values in $\mathbf{S}_{e}$ as the weight prior. For each column $\mathbf{y}$ of the noisy PG $\mathbf{Y}$, its sparse coefficients on dictionary $\mathbf{D}$ is obtained by solving
\vspace{-2mm}
\begin{equation}\label{equ11}\vspace{-2mm}
\hat{\boldsymbol{\upalpha}}:=\arg\min\nolimits_{\boldsymbol{\upalpha}}\|\mathbf{y}-\mathbf{D}\boldsymbol{\upalpha}\|_{2}^{2}+\sum_{i=1}^{3p^{2}}\lambda_{i}|\boldsymbol{\upalpha}_{i}|,
\end{equation}
where $\lambda_{i}$ is the $i$th regularization parameter defined as
\vspace{-2mm}
\begin{equation}\label{equ12}\vspace{-2mm}
\lambda_{i} = \lambda/(\sqrt{\mathbf{S}_{i}}+\varepsilon).
\end{equation}
We add a small positive number $\varepsilon$ to avoid zero denominator. Since the learned dictionary $\mathbf{D}$ is orthonormal, the Equ. (\ref{equ11}) also has a closed-form solution \cite{pgpd} 
\vspace{-2mm}
\begin{equation}\label{equ13}\vspace{-2mm}
\hat{\boldsymbol{\upalpha}}= \text{sgn}(\mathbf{D}^{T}\mathbf{y})\odot \text{max}(|\mathbf{D}^{T}\mathbf{y}|-\mathbf{\Lambda},\mathbf{0}),
\end{equation}
where $\mathbf{\Lambda} = [\lambda_{1},\lambda_{2},...,\lambda_{3p^2}]$ is the vector of regularization parameter and $\text{sgn}(\bullet)$ is the sign function, $\odot$ means element-wise multiplication, and $|\mathbf{D}^{T}\mathbf{y}|$ is the absolute value of each entry of vector $|\mathbf{D}^{T}\mathbf{y}|$. The closed-form solution makes our denoising process very efficient. 
