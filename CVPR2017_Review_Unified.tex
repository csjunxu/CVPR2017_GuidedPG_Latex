...
\end{split}
\end{equation} 
We maximize the above objective function via EM algorithm \cite{em} and finally obtain the GMM model with learned parameters as the learned external PG prior. Similar to \cite{pgpd}, the mean vector of each cluster is natural zeros, i.e., $\boldsymbol{\upmu}_{k}=\mathbf{0}$.
...
Now each internal noisy PG has been assigned to its most suitable Gaussian component $\mathcal{N}(\mathbf{0},\mathbf{\Sigma}_{k})$ (subspace). In order to recover the latent clean PG, we employ the sparse coding framework under which we need a suitable dictionary for coding. As we know, given a dictionary $\mathbf{D}$, its \emph{mutual incoherence} $\mu(\mathbf{D})$ \cite{donoho2003optimally}, which is difined as
\vspace{-2mm}
\begin{equation}\label{equ4}
\vspace{-2mm}
\mu(\mathbf{D})=\max_{i~=j}\frac{|\mathbf{d}_{i}^{T}\mathbf{d}_{j}|}{\|\mathbf{d}_{i}\|_{2}\|\mathbf{d}_{j}\|_{2}},
\end{equation}
is a measure of quality of dictionary. Since the \emph{mutual incoherence} of a orthogonal dictionary is $0$, its quality is often better than non-orthogonal dictionaries. 

external subspace through covariance matrix via singular value decomposition (SVD) 
\begin{equation}\label{equ4}
\mathbf{\Sigma}=\mathbf{U}_{e}\mathbf{S}_{e}\mathbf{V}_{e}^{T}.
\end{equation}
can learn for each subspace an internal orthogonal dictionary adaptive to the noisy PGs. For notation simplicity, we ignore the index of subspace $k$.
