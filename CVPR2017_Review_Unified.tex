...
\caption{Denoised images of a region cropped from the real noisy image ``Nikon D800 ISO 3200 A3" \cite{crosschannel2016} by different methods. The mean image of 500 shots of the static scene (with the same camera and camera setting) is roughly taken as the ``ground truth". The images are better viewed by zooming in on screen.} 
\vspace{-1mm}
\label{fig1}
\end{figure*}
...
\centering
\includegraphics[width=0.91\linewidth]{Flowchart.pdf}
\vspace{-27mm}
...
\end{equation}
where $\mathbf{S}_{k}(j)$ is the $j$-th singular value of diagonal singular value matrix $\mathbf{S}_{k}$ (please refer to Eq.\ (\ref{equ2})) and $\varepsilon$ is a small positive number to avoid zero denominator.\ Noted that $\mathbf{D}_{\text{E}}=\mathbf{U}_{k}$ if $r=3p^{2}$ and $\mathbf{D}_{\text{E}}=\emptyset$ if $r=0$.
...
\caption{Denoised images of a region cropped from the real noisy image ``Canon 5D Mark 3 ISO 3200 1" by different methods. The images 
\vspace{-0.5mm}
