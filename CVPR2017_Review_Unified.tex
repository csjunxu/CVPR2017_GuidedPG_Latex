...
Our proposed denoising method contains two stages, the external prior learning stage and the external prior guided internal learning stage. In the first stage, there are 4 parameters: the $p$ is set as $p = 6$ (hence the patch size is $6\times 6 \times 3$), the number of patches in a patch group (PG) is set as $M=10$, the window size for PG searching is set as $W=31$, and the number of Gaussians in Gaussian Mixture Model (GMM) is set as $K=32$. We learn the external prior via GMM on about 3.6 million PGs extracted from the Kodak PhotoCD Dataset (\url{http://r0k.us/graphics/kodak/}), which includes 24 high quality color images. In the second stage, there are 4 parameters: the number of internal atoms in the learned dictionaries is set as $r=3p^2/2=54$, the sparse regularization parameter is set as $\lambda=0.001$, the number of iterations for solving problem (\ref{equ5}) is set as $T=2$, and the number of iterations for the denoising algorithm is set as $IteNum=4$. Our proposed method needs about 60 seconds to denoise an image of size $512\times512\times3$ under the Matlab2014b environment on a machine with Intel(R) Core(TM) i7-5930K CPU of 3.5GHz and 32GB RAM.

\subsection{Comparison with External and Internal Priors}
In this section, we compare our proposed method on real image denoising with external prior based method (denoted as "External") and internal prior based method (denoted as "Internal"). All the three methods perform real image denoising under the weighted sparse coding framework in Equ. (\ref{equ5}). The "External" method employs the external dictionaries (i.e., $r=0$ in Equ. (\ref{equ5})) to recover the latent clean images, while the "Internal" method directly learn the subspace structures from the internal PGs via GMM and perform denoising with internal dictionary learning (i.e., $r=128$ in Equ. (\ref{equ5})). All parameters are tuned to achieve the best performance.

We compare the three methods on the 60 cropped images from \cite{crosschannel2016}. The average PSNR results of these methods are listed in Table \ref{tab1}. It can be seen that our proposed method achieves higher PSNR results then the "External" and "Internal" methods. We also compare the visual qualities of denoised images listed in Figure \ref{fig2} and Figure \ref{fig3}. We can see that the "External" method is good at recovering structures (Figure \ref{fig2}) while the "Internal" method is good at recovering complex textures (Figure \ref{fig3}). And if we make use of both the external and internal priors, our proposed method can recover well both the structures and textures.

In summary, the proposed algorithm achieves better performance on denoising real images than the external and internal prior based methods. 
...
In summary, our proposed algorithm demonstrates impressing ability on dealing with the complex noise in real noisy images in terms of PSNR and visual quality.
