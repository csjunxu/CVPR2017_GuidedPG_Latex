...
\begin{split}
&\min_{\mathbf{D}\in \mathbb{R}^{3p^2\times r},\textbf{A}\in\mathbb{R}^{3p^2\times MN}}
...
\textbf{Updating Weighted Sparse Coding}: given the initialization orthogonal dictioanry $\textbf{D}_{i}^{(t)}$, the sparce coefficients 
...
\textbf{D}_{i}^{(t+1)}:=
&
\argmin_{\textbf{A}\in\mathbb{R}^{3p^2\times MN}}
...
\begin{tabular}{l}
\hline
\textbf{Alg. 1}: External Guided Internal Dictionary Learning
\\
\hline
