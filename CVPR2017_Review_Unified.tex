...
Now all internal noisy PGs have been assigned to their corresponding most suitable Gaussians or subspaces in $\{\mathcal{N}(\mathbf{0},\mathbf{\Sigma}_{k})\}_{k=1}^{K}$. For each subspace, we employ its external orthogonal dictionary $\mathbf{D}_{e}$ to guide the internal orthogonal dictionary learning. The learning is performed under the sparse coding framework (along with which the denoising of real noisy image is simultaneously done) as follows:
\vspace{-2mm}
