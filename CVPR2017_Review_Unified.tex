...
{\footnotesize (d) Guided: \textbf{37.13}dB }
\end{minipage}
\begin{minipage}[t]{0.195\textwidth}
\centering
...
{\footnotesize (i) Guided: \textbf{40.26}dB}
\end{minipage}
\begin{minipage}[t]{0.195\textwidth}
\centering
...
In this section, we compare the proposed external prior guided internal prior learning framework with external prior method (denoted as "External") and internal prior method (denoted as "Internal") on real image denoising. Both methods perform real image denoising under the weighted sparse coding framework in Equ. (\ref{equ5}). The "External" method employs the external dictionaries (i.e., $r=0$ in Equ. (\ref{equ5})) to recover the latent clean images, while the "Internal" method directly utilizes the Gaussian Mixture Model to cluster the internal niosy PGs into multiple subspaces and perform denoising with internal dictionaries (i.e., $r=128$ in Equ. (\ref{equ5})). 

We compare the "External" method, the "Internal" method, and the proposed method (denoted as "Guided") on the first dataset provided in \cite{crosschannel2016}. We compare the PSNR and visual quality of these three methods. The average PSNR results on 60 cropped images from \cite{crosschannel2016} are listed in Table \ref{tab1}. It can be seen that the proposed "Guided" method achieves higher PSNR results then the "External" and "Internal" methods. We also compare the visual qualities of denoised images listed in Figure \ref{fig2} and Figure \ref{fig3}. We can see that the "External" method is good at recovering structures (Figure \ref{fig2}) while the "Internal" method is good at recovering complex textures (Figure \ref{fig3}). And if we make use of both the external and internal priors, the proposed "Guided" method can recover well both the structures and complex textures.
...
We compare with previous state-of-the-art Gaussian noise removal methods such as BM3D \cite{bm3d}, WNNM \cite{wnnm}, MLP \cite{mlp}, CSF \cite{csf}, and the recently proposed TRD \cite{chen2015learning}. We also compare with three competing real image denoising methods such as Noise Clinic, Neat Image, and the CCNoise method proposed recently. Neat Image \cite{neatimage} is a commercial software embeded in Photoshop CC and Corel PaintShop Pro X9 \cite{neatimage}. It first estimates the parameters of noise via a large flat area and then filters the noise accordingly. All these methods need noise estimation which is vary hard to perform if there is no uniform regions are available in the testing image. The NeatImage will fail to perform automatical parameters settings if there is no uniform regions. \footnote{To compare with CCNoise, we first transform the denoised images into double format.}
