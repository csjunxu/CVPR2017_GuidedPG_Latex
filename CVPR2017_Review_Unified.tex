...
\textbf{Updating Sparse Coefficients}: given the orthogonal dictioanry $\textbf{D}_{i}^{(t)}$, we update the sparce coefficients via solving
\vspace{-2mm}
...
\end{split}
\end{equation}
The sparse coefficients $\mathbf{A}=[\mathbf{A}_{e}^{T}\ \mathbf{A}_{i}^{T}]^{T}$ also has two parts: the external part $\mathbf{A}_{e}$ and the internal part $\mathbf{A}_{i}$ denote the coefficients over external dictionary $\mathbf{D}_{e}$ and internal dictionary $\mathbf{D}_{i}$, respectively. According to the Proposition 2.2 in \cite{bao2013fast}, the problem (\ref{equ6}) 
%\begin{equation}
%\begin{split}
...
\vspace{-3mm}
\begin{equation}\label{equ10}\vspace{-3mm}
\mathbf{D}^{T}\mathbf{D} = 
\left[\begin{array}{c}
\mathbf{D}_{e}^{T}
\\
