...
\vspace{-0.1in}
\item More denoising results on the 20 real noisy imagessupp (with no ``ground truth") provided in the dataset \cite{ncwebsite}.
\vspace{-0.1in}
\item More denoising results on the 15 smaller real noisy imagessupp (with ``ground truth") used in the dataset \cite{crosschannel2016}.
\vspace{-0.1in}
\item More denoising results on the 60 real noisy imagessupp (with ``ground truth") cropped from \cite{crosschannel2016}.
\end{enumerate}
...
{\footnotesize (a) Noisy Image \cite{ncwebsite}  }
\end{minipage}
\begin{minipage}[t]{0.33\textwidth}
\centering
...
\begin{minipage}[t]{0.33\textwidth}
\centering
\raisebox{-0.15cm}{\includegraphics[width=1\textwidth]{imagessupp/resize_frog_csf.png}}
{\footnotesize (e) CSF \cite{csf} }
\end{minipage}
\begin{minipage}[t]{0.33\textwidth}
\centering
\raisebox{-0.15cm}{\includegraphics[width=1\textwidth]{imagessupp/resize_frog_tnrd.png}}
{\footnotesize (f) TNRD \cite{chen2015learning}  }
\end{minipage}
}\vspace{-3mm}
\subfigure{
\begin{minipage}[t]{0.33\textwidth}
...
{\footnotesize (h) Noise Clinic \cite{noiseclinic,ncwebsite} }
\end{minipage}
\begin{minipage}[t]{0.33\textwidth}
\centering
...
\caption{
\vspace{-0.3mm}
(a) A real noisy image captured by a digital camera \cite{ncwebsite}. (b)$\sim$(i) Denoised imagessupp by different methods.
\vspace{-0.3mm}
Among them, CBM3D \cite{bm3d,cbm3d} and WNNM \cite{wnnm} are prior model based methods; MLP \cite{mlp}, CSF \cite{csf} and TNRD 
\vspace{-0.3mm}
\cite{chen2015learning} are discriminative learning based methods; and Neat Image \cite{neatimage} and Noise Clinic \cite{noiseclinic,ncwebsite} are methods designed 
\vspace{-0.3mm}
for real noisy imagessupp. Clearly, our method produces a cleaner image.
}
\vspace{-8mm}
\label{fig1}
\end{figure}
