...
For notation simplicity, we ignore the index $n,m,t$ in the sparse coding problem (6) in the main paper. And it turns into the following weighted sparse coding problem:
\vspace{-4mm}
...
\end{equation}
For simplicity, we denote $\mathbf{z} = \mathbf{D^{T}y}$. Here we have $\lambda_{j}\ge0$, $j=1,...,3p^2$, then problem (\ref{equ2}) can be written as
\vspace{-2mm}
\begin{equation}\label{equ3}\vspace{-2mm}
\min_{\boldsymbol{\alpha}}\sum_{j=1}^{3p^2}((\mathbf{z}_{j}-\bm{\alpha}_{j})^{2}+\lambda_{j}|\bm{\alpha}_{i}|).
\end{equation}
...
\hat{\bm{\alpha}}= \text{sgn}(\mathbf{D^{T}y})\odot \text{max}(|\mathbf{D^{T}y}|-\bm{\lambda}/2,0),
\end{equation}
